...
export default function CatalogueSelector ({shouldCache = false, autoSelectDefault = 'Aeldari - Aeldari Library'}) {
  const [loading, setLoading] = useState(true);
  const [gameSystemRepositoryData, setGameSystemRepositoryData] = useState(null);
  const [gameSystem] = useContext(GameSystemContext)
  const [catalogue, setCatalogue] = useContext(CatalogueContext)

  useEffect(() => {
    if (gameSystemRepositoryData && gameSystemRepositoryData.name === gameSystem.name) {
      setLoading(false);
      return
    }
      ...
      if (shouldCache) {
        ...
        if (localStorageCache) {
          ...
          if (parsedCache.repositoryUrl === gameSystem.repositoryUrl) {
            ...
            console.log('loading gameSystemRepositoryData from localStorage cache', gameSystemRepositoryData)
          } else {
            console.log('INVALIDATING CACHE: existing gameSystemRepositoryData localStorage cache does not match selected gameSystem', parsedCache.repositoryUrl, gameSystem.repositoryUrl)
          }
        }
      }
      if (!gameSystemRepositoryData) {
        gameSystemRepositoryData = await (await fetch(`https://api.codetabs.com/v1/proxy/?quest=${gameSystem.repositoryUrl}`)).json()
        if (shouldCache) {
          window.localStorage.setItem(localStorageCacheKey, JSON.stringify(gameSystemRepositoryData))
          console.log('caching gameSystemRepositoryData from network in localStorage', gameSystemRepositoryData)
        } else {
          console.log('shouldCache disabled, fetched gameSystemRepositoryData from network', gameSystemRepositoryData)
        }
      }
      setGameSystemRepositoryData(gameSystemRepositoryData);
      setLoading(false);
      if (autoSelectDefault) {
        const defaultSelection = gameSystemRepositoryData.repositoryFiles.find((file) => file.name === autoSelectDefault)
        if (defaultSelection) {

...
import gameSystemSchema from './schemas/gameSystem.schema.json';
import sharedSchema from './schemas/shared.schema.json';
import containerTags from './schemas/containerTags.json';
...
const unescape = (string) =>
  escapedHtml.test(string)
    ? string.replace(escapedHtml, (match) => htmlUnescapes[match])
    : string;
